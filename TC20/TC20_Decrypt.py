'''
==============================
TC20 - Toy Cipher (Decryption)
==============================
'''

Sbox_inv = []
# -----------------
# AR : AddRoundKey (Xor)
def AR(in_state, rkey):
        out_state = [0, 0, 0, 0]
        for i in range(len(in_state)):
                out_state[i] = in_state[i] ^ rkey[i]
        return out_state

# -----------------
# SB : Sbox layer
def invSB(in_state):
        out_state = [0, 0, 0, 0]
        for i in range(len(in_state)):
                out_state[i] = Sbox_inv[in_state[i]] # 비선형 치환
        return out_state

# -----------------
# LM: Linear Map
def invLM(in_state):
        out_state = [0, 0, 0, 0]
        all_xor = in_state[0] ^ in_state[1] ^ in_state[2] ^ in_state[3]
        for i in range(len(in_state)):
                out_state[i] = all_xor ^ in_state[i]
        return out_state

# -----------------
# Enc_Round : Encrypt Round
def Dec_Round(in_state, rkey):
        out_state = AR((invSB(invLM(in_state))), rkey)
        return out_state

# -----------------
# Tc20_Enc : Encryption
def TC20_Dec(CT, key):
        PT = CT
        NROUND = 10 # 라운드 횟수 = 10
        for i in range(0, NROUND):
            PT = Dec_Round(PT, key) # 라운드 키 = 암호 키
        return PT
# -----------------
# main()
def main():
        print("Decryption Test Starts...")
        key = [0, 1, 2, 3]
        CT = [202, 134, 119, 230]
        PT = TC20_Dec(CT, key) # 리스트로 출력
        B_PT = bytes(PT)

        PT = ''.join([chr(ch) for ch in PT])

        print('CT = ', CT)
        print('Key = ', key)
        print('PT = ', PT)
        print('B_PT = ', B_PT)
        print("Decryption Completed...")
##-- Run main()

if __name__ == '__main__':
    main()