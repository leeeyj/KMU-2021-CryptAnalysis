'''
==============================
TC20 - Toy Cipher (Encryption)
==============================
'''

Sbox = []

# -----------------
# AR : AddRoundKey (Xor)
def AR(in_state, rkey):
        out_state = [0, 0, 0, 0]
        for i in range(len(in_state)):
                out_state[i] = in_state[i] ^ rkey[i]
        return out_state

# -----------------
# SB : Sbox layer
def SB(in_state):
        out_state = [0, 0, 0, 0]
        for i in range(len(in_state)):
                out_state[i] = Sbox[in_state[i]] # 비선형 치환
        return out_state

# -----------------
# LM: Linear Map
def LM(in_state):
        out_state = [0, 0, 0, 0]
        all_xor = in_state[0] ^ in_state[1] ^ in_state[2] ^ in_state[3]
        for i in range(len(in_state)):
                out_state[i] = all_xor ^ in_state[i]
        return out_state

# -----------------
# Enc_Round : Encrypt Round
def Enc_Round(in_state, rkey):
        out_state = LM(SB(AR(in_state, rkey)))
        return out_state

# -----------------
# Tc20_Enc : Encryption
def TC20_Enc(PT, key):
        CT = PT
        NROUND = 10 # 라운드 횟수 = 10
        for i in range(0, NROUND):
                CT = Enc_Round(CT, key) # 라운드 키 = 암호 키
        return CT
# -----------------
# main()
def main():
        print("Encryption Test Starts...")
        in_msg = "ARIA"
        key = [0, 1, 2, 3]
        PT = [ord(ch) for ch in in_msg]
        CT = TC20_Enc(PT, key)

        B_CT = bytes(CT)


        print('PT = ', PT)
        print('Key = ', key)
        print('CT = ', CT)
        print('B_CT = ', B_CT)
        print("Encryption Completed...")
##-- Run main()

if __name__ == '__main__':
    main()